...
# Run separately for each of three Evo2 context window sizes: 8,192 bp /
# 16,384 bp / 65,536 bp, producing the same set of plots for each (filenames
# suffixed with e.g. "_16384bp").
#
...
# Evo2 7b-arc-longcontext delta scores, one file per context window size.
# The pipeline below (run_context_analysis) is executed once per entry.
evo2_context_configs <- list(
  list(
    context_bp = 8192,
    evo2_file  = "C:/Users/mitch/Documents/Argonne/Variant_Scoring/scored_MVP_results/MVP_variant_scores.7b_arc_longcontext_model.8192bp_context.csv"
  ),
  list(
    context_bp = 16384,
  ...
  list(
    context_bp = 65536,
    evo2_file  = "C:/Users/mitch/Documents/Argonne/Variant_Scoring/scored_MVP_results/MVP_variant_scores.7b_arc_longcontext_model.65536bp_context.csv"
  )
)
...
# ============================================================================
# STEP 0: LOAD FUNCTIONAL ANNOTATIONS (shared across all Evo2 context windows)
# ============================================================================

message("Loading functional annotations...")
df_func_raw <- read_csv(func_file, show_col_types = FALSE)

# ============================================================================
# STEP 1: LOAD AND MERGE DATA (per Evo2 context window)
# ============================================================================
# Everything from here down is wrapped in run_context_analysis() so it can be
# repeated identically for each Evo2 context window size (8,192 / 16,384 /
# 65,536 bp), producing the same set of plots for each.

run_context_analysis <- function(evo2_file, context_bp, df_func, output_dir) {
  
  # Human-readable, comma-formatted context window label, e.g. "16,384 bp"
  context_label    <- paste0(format(context_bp, big.mark = ",", scientific = FALSE), " bp")
  evo2_model_label <- paste0("Evo2 Delta Score\n(7b-arc-longcontext, ", context_label, ")")
  # Plain-digit suffix (no commas) for use in output filenames
  file_suffix      <- paste0("_", context_bp, "bp")
  
  message("\n============================================================================")
  message("Running Evo2 analysis for context window: ", context_label)
  message("============================================================================")
  
  message("Loading Evo2 scores (7b-arc-longcontext, ", context_label, ")...")
  ...
  allvar_preds <- list(
    ...
    evo2_delta_score = list(col = "evo2_delta_score",  label = evo2_model_label)
  )
  ...
  ggsave(file.path(output_dir, paste0("functional_individual_regressions_allvar", file_suffix, ".png")),
         plot_violins_allvar, width = 15, height = 5, dpi = 300)
  message("Saved: functional_individual_regressions_allvar", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_joint_regression_forest_allvar", file_suffix, ".png")),
         plot_forest_allvar, width = 12, height = 5, dpi = 300)
  message("Saved: functional_joint_regression_forest_allvar", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_vif_allvar", file_suffix, ".png")),
         plot_vif_allvar, width = 8, height = 5, dpi = 300)
  message("Saved: functional_vif_allvar", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_neartranscript_regressions", file_suffix, ".png")),
         plot_nt_combined, width = 15, height = 14, dpi = 300)
  message("Saved: functional_neartranscript_regressions", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_missense_violins", file_suffix, ".png")),
         plot_violins_missense, width = 16, height = 10, dpi = 300)
  message("Saved: functional_missense_violins", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_missense_forest", file_suffix, ".png")),
         plot_forest_missense, width = 14, height = 5, dpi = 300)
  message("Saved: functional_missense_forest", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_vif_missense", file_suffix, ".png")),
         plot_vif_missense, width = 9, height = 5, dpi = 300)
  message("Saved: functional_vif_missense", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_categorical_plots", file_suffix, ".png")),
         plot_categorical, width = 14, height = 6, dpi = 300)
  message("Saved: functional_categorical_plots", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_pairwise_correlations_allvar", file_suffix, ".png")),
         plot_corr_allvar, width = 15, height = 5, dpi = 300)
  message("Saved: functional_pairwise_correlations_allvar", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_missense_corr_heatmap", file_suffix, ".png")),
         plot_heatmap, width = 9, height = 8, dpi = 300)
  message("Saved: functional_missense_corr_heatmap", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_maf_evo2_correlation", file_suffix, ".png")),
         plot_maf_evo2, width = 16, height = 6, dpi = 300)
  message("Saved: functional_maf_evo2_correlation", file_suffix, ".png")
  ...
  ggsave(file.path(output_dir, paste0("functional_maf_regression_forest", file_suffix, ".png")),
         plot_maf_forest, width = 13, height = 5, dpi = 300)
  message("Saved: functional_maf_regression_forest", file_suffix, ".png")
  
  message("\n=== Done: ", context_label, " context window ===")
  
} # end run_context_analysis()

# ============================================================================
# RUN THE FULL PIPELINE FOR EACH EVO2 CONTEXT WINDOW SIZE
# ============================================================================
# Produces the same set of twelve plots for each of the 8,192 / 16,384 /
# 65,536 bp context windows, distinguished by a "_<context>bp" filename
# suffix (e.g. functional_missense_forest_65536bp.png).
...
walk(evo2_context_configs, function(cfg) {
  run_context_analysis(
    evo2_file  = cfg$evo2_file,
    context_bp = cfg$context_bp,
    df_func    = df_func_raw,
    output_dir = output_dir
  )
})

message("\n=== All Done: all context windows complete ===")